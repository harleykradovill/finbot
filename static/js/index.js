document.addEventListener('DOMContentLoaded', () => {
  ...
  async function loadActivity(days = 365) {
    try {
      ...
      while (page <= maxPages) {
        const resp = await fetch(`/api/analytics/activitylog?page=${page}&per_page=${perPage}`);
        if (!resp.ok) break;
  ...
  function buildMatrix(items, days = 365) {
    ...
    items.forEach(it => {
      ...
      if (!ts) return;
      const d = new Date(ts);
      const dayDiff = Math.floor((Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate()) -
        Date.UTC(d.getUTCFullYear(), d.getUTCMonth(), d.getUTCDate())) / msPerDay);
  ...
  async function render() {
    ...
    if (window.__playsMatrixChart) {
      try { window.__playsMatrixChart.destroy(); } catch (e) {}
